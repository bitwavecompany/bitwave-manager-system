    ...
    <div class="login-card bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-[#334155] rounded-2xl md:rounded-xl shadow-lg p-12 md:p-10 max-w-md w-full relative z-10">
      ...
      <form class="login-form flex flex-col gap-6 md:gap-6" @submit.prevent="handleLogin">
        <div class="input-group flex flex-col gap-2">
          <label for="username" class="input-label text-sm font-semibold text-gray-700 dark:text-white dark:font-bold ml-1">Correo</label>
      ...
      <p class="register-link text-center text-sm text-gray-500 dark:text-slate-400 mt-4">¿No tienes una cuenta?
        <a rel="noopener noreferrer" href="#" class="register-button text-blue-500 font-semibold ml-1 px-2 py-1 rounded transition-colors focus-visible:outline focus-visible:outline-blue-500" @click.prevent="router.push('/register')">Regístrate</a>
      </p>
    </div>
  </div>
</template>
...
async function handleLogin(e: Event) {
  ...
  try {
    ...
    if (data?.errors) {
      // Convierte los errores de validación en un array de strings
